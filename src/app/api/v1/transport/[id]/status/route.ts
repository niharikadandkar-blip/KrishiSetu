...
import { transitionTransportStatusSchema } from '@/lib/validations/phase6';
...
export async function POST(
  ...
  try {
    ...
    const validation = transitionTransportStatusSchema.safeParse({ ...body, requestId, userId });
    ...
    const updatedRequest = await transportRepository.transitionTransportStatus(validation.data);

    return NextResponse.json({
      success: true,
      request: updatedRequest,
      message: `Transport status updated to '${validation.data.targetStatus}' successfully.`,
    });
  } catch (error: any) {
    ...
    if (errorMsg.startsWith('TRANSPORT_NOT_FOUND')) {
      status = 404;

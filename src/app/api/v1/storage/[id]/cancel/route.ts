...
import { cancelStorageSchema } from '@/lib/validations/phase6';
...
export async function POST(
  ...
  try {
    ...
    const validation = cancelStorageSchema.safeParse({ ...body, requestId, userId });
    ...
    const cancelledRequest = await storageRepository.cancelStorageRequest(validation.data);

    return NextResponse.json({
      success: true,
      request: cancelledRequest,
      message: 'Storage reservation cancelled and capacity released successfully.',
    });
  } catch (error: any) {

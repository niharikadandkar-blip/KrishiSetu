...
export default function BuyerOffersPage() {
  ...
  // Counter Modal State (Buyer responding to Farmer Counter)
  ...
  const fetchSubmittedOffers = async () => {
    ...
    setMessage(null);
    try {
      const res = await fetch(`/api/v1/offers?userId=${user.id}&role=BUYER`);
  ...
  const handleWithdrawOffer = async (bidId: string) => {
    setMessage(null);
    try {
      const res = await fetch(`/api/v1/offers/${bidId}/withdraw`, {
        method: 'POST',
  ...
  const handleRespondToCounter = async (bidId: string, action: 'ACCEPT' | 'REJECT' | 'COUNTER') => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        if (action === 'ACCEPT') {
          ...
          setMessage({
            type: 'success',
            text: data.message || t('offers.counterSubmittedSuccess'),
          });
        }
        fetchSubmittedOffers();
      } else {
  ...
  const handleSubmitCounterResponse = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok && data.success) {
        setMessage({ type: 'success', text: t('offers.counterSubmittedSuccess') });
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-3 mb-6 scrollbar-none border-b border-slate-200 dark:border-slate-700/60">
          {[
            ...
            { id: 'WITHDRAWN', label: t('offers.tabWithdrawn') },
          ].map((tab) => (
            <button
              key={tab.id}
                ...
                >
                  <div>
                    <div className="flex items-start justify-between gap-2 mb-3">
                      <div>
                        ...
                        <p className="text-xs text-slate-500 flex items-center gap-1 mt-0.5">
                          ...
                          {offer.lot?.publicVillage}, {offer.lot?.publicDistrict} • Farmer: {offer.lot?.farmer?.name || 'Farmer'}
                        </p>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs bg-slate-50 dark:bg-slate-900/60 p-3.5 rounded-xl mb-4 border border-slate-100 dark:border-slate-800">
                      <div>
                        <span className="text-slate-500 block">Your Offered Price</span>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {isPending && (
                        <button
                          ...
                          className="px-3 py-1.5 rounded-xl bg-rose-50 text-rose-700 hover:bg-rose-100 dark:bg-rose-950 dark:text-rose-300 text-xs font-semibold flex items-center gap-1 transition-all"
                        >
                          <Ban className="w-3 h-3" />
                          {t('offers.actionWithdraw')}
                        </button>
                      )}
                      ...
                      {isCountered && (
                        <>
                          ...
                          <button
                            onClick={() => handleRespondToCounter(offer.id, 'COUNTER')}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white dark:bg-slate-800 rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-700 space-y-4">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white">
              Respond to Farmer Counter Offer
            </h3>

            <form onSubmit={handleSubmitCounterResponse} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">
                  Your New Counter Price (₹ per {selectedOfferForCounter.lot?.unit})
                </label>
                <input
                  type="number"
                  min="1"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">
                  Your New Quantity ({selectedOfferForCounter.lot?.unit})
                </label>
                <input
                  type="number"
                  min="1"
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                ...
                >
                  {submittingResponse ? t('common.loading') : 'Send Counter'}
                </button>
              </div>
            </form>
          </div>
            ...
            {loadingTimeline ? (
              ...
              <div className="space-y-4 py-2 max-h-80 overflow-y-auto">
                {timelineEvents.map((evt) => (
                  <div key={evt.id} className="relative pl-6 border-l-2 border-amber-500/40 space-y-1">
                    <div className="absolute -left-1.5 top-0 w-3 h-3 rounded-full bg-amber-500 ring-4 ring-white dark:ring-slate-800" />
